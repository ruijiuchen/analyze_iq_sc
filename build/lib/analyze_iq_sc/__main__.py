...
def start_generate_full_spectrum_worker(folder_entry_dic, file_listbox_dic, button_dic,should_stop_online_server, default_path_injection_files, frameID_offset, frameID_range):
        ...
        while not should_stop_online_server[0]: 
                ...
                file_listbox_injection_files  = file_listbox_dic.get('injection_files', None)
                update_file_list(file_listbox_injection_files,default_path_injection_files,default_path_injection_files,"analyzed_files_iq.txt",".root")  # Update the file list in file_listbox_injection_files with default path
                sort_listbox(file_listbox_injection_files) 
                time.sleep(60)
...
def sort_listbox(listbox):
        # Retrieve items and their colors
        items = list(listbox.get(0, tk.END))
        colors = [get_listbox_item_color(listbox, i) for i in range(len(items))]
        
        # Combine items and colors into a list of tuples and sort by item name
        items_with_colors = sorted(zip(items, colors), key=lambda x: x[0])
        
        # Clear the listbox and re-insert items in sorted order with their respective colors
        listbox.delete(0, tk.END)
        for item, color in items_with_colors:
                listbox.insert(tk.END, item)
                set_listbox_iterm_color(listbox, listbox.size() - 1, color)
...
def main():
        ...
        root = tk.Tk()
        root.title("Convert IQ data to frequency spectrum.")
        ...
        folder_entry_synced_files_iq = tk.Entry(root,width=100, justify='right')
        folder_entry_synced_files_iq.grid(row=0, column=1, columnspan=4,padx=10, pady=10, sticky="w")  # Right-aligned
        ...
        comment_label_analyzed_files_iq.grid(row=1, column=0, padx=10, pady=10, sticky="w") 
        # Create an Entry widget for analyzed_files_iq
        folder_entry_analyzed_files_iq = tk.Entry(root,width=100, justify='right')
        folder_entry_analyzed_files_iq.grid(row=1, column=1, columnspan=4,padx=10, pady=10, sticky="w")  # Right-aligned
        ...
        comment_label_synced_files_sc.grid(row=2, column=0, padx=10, pady=10, sticky="w")  # Right-aligned
        # Create an Entry widget for synced_files_sc
        folder_entry_synced_files_sc = tk.Entry(root,width=100, justify='right')
        ...
        comment_label_analyzed_files_sc.grid(row=3, column=0, padx=10, pady=10, sticky="w")  # Right-aligned
        # Create an Entry widget for analyzed_files_sc
        folder_entry_analyzed_files_sc = tk.Entry(root,width=100, justify='right')
        ...
        browse_button_synced_files_iq.grid(row=0, column=4, padx=0, pady=10, sticky="e")  # Left-aligned
        
        ## Create a "Browse" button for folder selection
        browse_button_analyzed_files_iq = tk.Button(root, text="Browse", command=lambda: browse_folder_and_update_list_filebox(folder_entry_analyzed_files_iq,default_path_analyzed_files_iq,default_path_analyzed_files_iq,file_listbox_analyzed_files_iq,"analyzed_files_iq.txt",".iq.tdms.bin_amp"))
        browse_button_analyzed_files_iq.grid(row=1, column=4, padx=0, pady=10, sticky="e")  # Left-aligned
        ...
        browse_button_synced_files_sc.grid(row=2, column=4, padx=0, pady=10, sticky="e")  # Left-aligned
    
        # Create a "Browse" button for folder selection
        browse_button_analyzed_files_sc = tk.Button(root, text="Browse", command=lambda: browse_folder_and_update_list_filebox(folder_entry_analyzed_files_sc,default_path_analyzed_files_sc,default_path_analyzed_files_sc,file_listbox_analyzed_files_sc,"analyzed_files_sc.txt",".sc.tdms.root"))
        browse_button_analyzed_files_sc.grid(row=3, column=4, padx=0, pady=10, sticky="e")  # Left-aligned
